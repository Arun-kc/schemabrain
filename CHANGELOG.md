...
### Security
- `schemabrain serve` defaults `--pii-block` to `{credential, payment_card, government_id}` when the flag is absent (previously empty frozenset — no enforcement). Surfaces a one-line stderr confirmation so the choice is visible. Pass `--pii-block ''` to explicitly disable enforcement (an explicit-empty branch with its own warning). ([#104])
