"""Test isolation for the data-dictionary suite.

The CLI tests here invoke `schemabrain.cli.main()`, which calls
`configure_logging` — a function that sets the `schemabrain` package
logger's ``propagate = False`` and swaps in a stderr handler. That is a
global, process-wide side effect. Because this directory sorts before
...
order-independent and never pollutes the rest of the run.
"""
