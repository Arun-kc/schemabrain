...
| `test_fw_005_aggregate_pii_leak.py` | FW-005 | FIXED | HIGH | `_resolve_pii_categories` now refuses `MIN`/`MAX` over any column with non-empty PII categories, before plan emission. Refusal fires independent of `--pii-block` because MIN/MAX of a tagged column is row-level disclosure, not aggregation |
...
| `test_sf_002_propagation_empty_fail_open.py` | SF-002 | FIXED | HIGH | `propagate([])` retains its public-baseline contract; the fail-closed check moved to `_resolve_pii_categories` — when `--pii-block` is active AND the tag-table lookup returns zero rows, the call refuses rather than silently degrading to confirmed-clean |
