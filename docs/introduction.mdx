...
<Steps>
  ...
  <Step title="Initialize">
    ...
    schemabrain init
    ```
    The wizard targets Claude Desktop by default; pass `--host claude-code` / `cursor` / `windsurf` to wire a different MCP host. It spins up a local Postgres demo with a 7-table e-commerce schema and writes a ready-to-use config. Press Enter at every prompt for the demo defaults.
  </Step>
