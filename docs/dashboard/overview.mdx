...
1. **Which entities carry catastrophic-leak PII categories?** — see [PII Ledger](/dashboard/pii-ledger).
...
<Frame caption="PII Ledger — which entities carry catastrophic PII, and what the default policy blocks.">
  <img src="/assets/dashboard-pii-ledger.png" alt="PII Ledger dashboard surface" />
</Frame>

<Frame caption="Refusals — every blocked call, the reason that fired, and the recovery hint the agent received.">
  <img src="/assets/dashboard-refusals.png" alt="Refusals dashboard surface" />
</Frame>

<Frame caption="Audit Viewer — the tamper-evident chain, re-walked and verified server-side down to each row's hash linkage.">
...
| `GET /api/entities/pii-matrix` | Per-entity PII category counts + totals for the PII Ledger surface. |
...
| `GET /api/audit/verify` | Re-walks the chain hash; returns `intact` or `mismatch`. |
...
- `X-Schemabrain-Dashboard-Schema: 1.0` — the dashboard JSON contract.
