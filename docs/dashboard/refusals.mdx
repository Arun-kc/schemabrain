...
<Frame caption="Refusals — a chronological list of refused calls on the left, the full envelope on the right: the reason that fired, the blocked category set, and the recovery hint the agent received.">
  <img src="/assets/dashboard-refusals.png" alt="Refusals dashboard surface — incident timeline with the full refusal envelope detail" />
</Frame>

Every tool call that ended with `status='refused'` lands on this surface. The page renders a two-pane layout: a chronological list of incidents on the left, the full envelope detail (what the agent saw, what reason fired, which categories the policy intersected) on the right.
...
2. **Recovery quality review.** Read the envelopes SchemaBrain returned. Were the `follow_up_hints` actually useful? Did the agent self-correct on the next turn?
...
The detail panel adds the full envelope body: status, `follow_up_hints`, the structured recovery payload the agent received, and the `chain_hash` that anchors the row in the audit log.
...
A refusal is **not** an error. It is a Charter-conformant envelope with `status='refused'` and a structured `follow_up_hint`. The agent receives enough information to either:
...
The six refusal reasons each map to a recovery path documented in [Structured Recovery](/mechanism/structured-recovery).
