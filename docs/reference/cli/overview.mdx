...
The local SQLite store defaults to `~/.schemabrain/store.db`. Pass `--store-path PATH` to override. The directory is created on first write; subsequent runs are idempotent.
