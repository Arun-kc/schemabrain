...
description: "Live interactive Premium architecture diagram — 4-mode scene switcher with animated SVG pulses on the agent ↔ SchemaBrain ↔ Postgres ↔ audit-chain loop."
---

The same architecture from [the static page](/architecture), rendered live. Use the 4 tabs at the top of the scene to switch between modes — happy-path flow, PII refusal, indexing pipeline, audit-chain proof.
...
- The scene auto-scales to fit the available width via its internal `fitScene()` JS.
- Cyber-dark palette is intentional — the original design is dark-mode-first; light theme would not work for this surface.
- For deeper context on each layer, see [the static Architecture page](/architecture) where the diagram is annotated with prose.
